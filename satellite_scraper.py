...
def run_once(out_dir="satellite_out", lookback_days=3):
    ...
    # ---------------- Copernicus ----------------
    try:
        raws = copernicus.search(bbox, start_iso, end_iso, [], top=100, aoi_ring=ring)
        kept = 0
        for raw in raws:
            rec = to_state_record(raw, now_iso)
            key = item_key(rec)
            if key not in state:
                ...
                new_count += 1
                kept += 1
        print(f"[Copernicus] 取得{len(raws)}件 / 新規{kept}件", file=sys.stderr)
    except Exception as e:  # noqa: BLE001
        print(f"[Copernicus] エラー: {e}", file=sys.stderr)
