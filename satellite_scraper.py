...
使い方:
  ...
  python satellite_scraper.py --once --lookback-days 365 --chunk-days 15   # 初回1年分バックフィル
"""
import argparse
import json
import os
...
def run_once(out_dir="satellite_out", lookback_days=3, chunk_days=15, request_delay=0.3):
    ...
    print(f"検索範囲: 過去{lookback_days}日分を{len(chunks)}回に分割して取得します（1回あたり約{effective_chunk_days}日分）", file=sys.stderr)
    ...
    for chunk_start, chunk_end in chunks:
        ...
        if mgp_key:
            try:
                raws = mgp.search(bbox, chunk_start, chunk_end, [], mgp_key, limit=100)
                kept = 0
                for raw in raws:
                    ...
                    if key not in state:
                        ...
                        new_count += 1
                        kept += 1
                print(f"[MGP] {chunk_start[:10]}〜{chunk_end[:10]}: 取得{len(raws)}件 / AOI内かつ新規{kept}件", file=sys.stderr)
            except Exception as e:  # noqa: BLE001
                print(f"[MGP] {chunk_start[:10]}〜{chunk_end[:10]}: エラー: {e}", file=sys.stderr)
            time.sleep(request_delay)
        elif chunk_start == chunks[0][0]:
            print("[MGP] MGP_API_KEY未設定のためスキップ", file=sys.stderr)

        # ---------------- Copernicus ----------------
        for collection in TARGET_COLLECTIONS:
            try:
                raws = copernicus.search(bbox, chunk_start, chunk_end, [collection], top=100, aoi_ring=ring)
                kept = 0
                for raw in raws:
                    if raw.get("sensor") == "-":
...
def main():
    ...
    ap.add_argument("--chunk-days", type=int, default=int(os.environ.get("CHUNK_DAYS", "15")),
                     help="この日数を超えるlookback-daysの場合、この日数単位に分割してAPIを叩く（APIの返却件数上限による取りこぼし防止）")
    args = ap.parse_args()
    run_once(args.out_dir, args.lookback_days, args.chunk_days)
