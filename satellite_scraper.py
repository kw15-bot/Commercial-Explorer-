...
def run_once(out_dir="satellite_out", lookback_days=3):
    ...
    start_iso = (now - timedelta(days=lookback_days)).strftime("%Y-%m-%dT%H:%M:%S.000Z")
    ...
    if mgp_key:
        try:
            raws = mgp.search(bbox, start_iso, end_iso, [], mgp_key, limit=100)
            kept = 0
            for raw in raws:
                ...
                if key not in state:
                    ...
                    new_count += 1
                    kept += 1
            print(f"[MGP] 取得{len(raws)}件 / AOI内かつ新規{kept}件", file=sys.stderr)
        except Exception as e:  # noqa: BLE001
            print(f"[MGP] エラー: {e}", file=sys.stderr)
    else:
        print("[MGP] MGP_API_KEY未設定のためスキップ", file=sys.stderr)

    # ---------------- Copernicus ----------------
    # コレクションを明示的に絞る（未指定だとSentinel-5P(1回の観測で
    # 10種近いファイルを生成)やCopernicus Global Land Service等の非衛星画像
    # プロダクトまで大量に混ざり、top=Nの枠をSentinel-1/2/3より先に埋めて
    # しまうため）。コレクションごとに個別クエリして公平に配分する。
    TARGET_COLLECTIONS = ["SENTINEL-1", "SENTINEL-2", "SENTINEL-3", "SENTINEL-5P", "SENTINEL-6"]
    for collection in TARGET_COLLECTIONS:
        try:
            raws = copernicus.search(bbox, start_iso, end_iso, [collection], top=50, aoi_ring=ring)
            kept = 0
            for raw in raws:
                if raw.get("sensor") == "-":
                ...
                if key not in state:
                    ...
                    new_count += 1
                    kept += 1
            print(f"[Copernicus:{collection}] 取得{len(raws)}件 / 新規{kept}件", file=sys.stderr)
        except Exception as e:  # noqa: BLE001
            print(f"[Copernicus:{collection}] エラー: {e}", file=sys.stderr)
...
def main():
    ...
    args = ap.parse_args()
    run_once(args.out_dir, args.lookback_days)
