...
MGP_PRO_PORTAL_URL = "https://pro.maxar.com"
...
def normalize(feature):
    ...
    stac_links = feature.get("links", []) or []

    # 運用上使うのは以下の2種類:
    #   ・サムネイル表示   -> assets.browse.href（プレビュー画像。低解像度で構わない
    #                        — 雲量の目視確認が目的のため）
    #   ・メタデータ確認   -> links[rel=self].href（STACアイテム自体のAPI URL、JSON）
    #
    # DLリンクは意図的に持たせていない。MGP Proの実際の画像取得は
    # 「Order Image」画面での有償・非同期の発注フロー（配送先指定、クレジット消費、
    # 納品まで2〜90日）であり、APIキーを付けてGETすれば即座に返ってくる類の
    # ものではないと判明したため（2026-08時点でUIから確認）。Discovery APIの
    # assetsは検索結果のプレビュー用メタデータに過ぎず、ここにフル解像度データへの
    # 直リンクを期待するのはそもそも設計上誤りだった。そのためワンクリックDLは
    # 実装せず、常にポータル（Vantor Hub、旧MGP Pro）の発注画面を開くリンクのみを
    # 提供する（links参照）。
    #
    # 補足: 特定の未発注アイテムを指してポータルを直接開く「ディープリンク」は
    # 存在しない（公式リリースノート確認済み）。あるのは発注【後】の追跡用の
    # ?order_id=<xxx> パラメータのみ（Downloadsタブが開く）で、これから注文したい
    # 画像を指定する用途には使えない。よってポータルは常にトップページを開く形で
    # 固定とし、該当画像の特定はユーザーがIDをコピーしてポータル内で検索する
    # 運用にしている（ビューア側のポップアップでIDを選択・コピーしやすくしてある）。
    thumbnail_url = (assets.get("browse") or {}).get("href")
    metadata_url = next(
        (l["href"] for l in stac_links if l.get("rel") == "self" and l.get("href")),
        None,
    )
    ...
    return {
        ...
        "download_url": None,  # MGPは常にNone（上記コメント参照）。ビューア側はポータルを開く導線のみを出す
        "links": [{"label": "Vantor Hub（旧MGP Pro）で開く", "url": MGP_PRO_PORTAL_URL}],
