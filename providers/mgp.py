...
def normalize(feature):
    ...
    #   ・DLリンク         -> フル解像度の実データ資産。ただし実際に観測されたSTAC
    #                        レスポンスには browse/cloud-cover/sample-point-set/self
    #                        しか含まれておらず、フル解像度資産を指すキーは
    #                        【未確認】（HANDOFF §5参照）。ここでは既知の非表示用
    #                        アセット（browse/cloud-cover/sample-point-set）以外の
    #                        アセットがあれば「それがフル解像度資産だろう」という
    #                        推測でdownload_urlに採用する。実データで実際に機能するか
    #                        は次回Actions実行後にstate.jsonを見て要確認。
    thumbnail_url = (assets.get("browse") or {}).get("href")
    metadata_url = next(
        (l["href"] for l in stac_links if l.get("rel") == "self" and l.get("href")),
        None,
    )
    _KNOWN_NON_DATA_ASSETS = {"browse", "cloud-cover", "sample-point-set"}
    download_url = next(
        (a["href"] for key, a in assets.items()
         if key not in _KNOWN_NON_DATA_ASSETS and a and a.get("href")),
        None,
    )
    ...
    return {
        ...
        "download_url": download_url,  # 未確認。無ければNoneのままでビューア側はポータルリンクにフォールバック
        "links": [{"label": "MGP Proで開く", "url": MGP_PRO_PORTAL_URL}],
