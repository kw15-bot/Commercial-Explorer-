...
def search(bbox, start_iso, end_iso, collections, top=100, timeout=30, aoi_ring=None, max_pages=5):
    ...
    params = {
        ...
        "$top": top,
        # Assetsも展開しないとサムネイル(Quicklook)情報がレスポンスに含まれない。
        # 以前はAttributesのみ展開していたため、normalize()側のQUICKLOOK抽出ロジックが
        # 常に空振りしていた（サムネイルが一切表示されない不具合の原因）。
        # 注意: CDSEの$expandは "Attributes,Assets" のようなカンマ区切りではなく、
        # $expand=Attributes&$expand=Assets のように同名パラメータを繰り返す方式でないと
        # 400 Bad Requestになる（公式ドキュメントのサンプルで確認）。requestsに渡す際は
        # 値をリストにすると自動的にこの形（同名キーの繰り返し）でシリアライズされる。
...
def normalize(product):
    ...
    # 運用上使うのは以下の3種類のみ:
    #   ・サムネイル表示 -> Assets配列中 Type が QUICKLOOK のものの DownloadLink
    #                     （CDSE公式ドキュメントで確認: Assets配列の各要素はDownloadLink
    #                     プロパティを持ち、そのURLで直接取得できる。$expand=Assetsを
    #                     指定しないとレスポンスに含まれない点に注意 — search()参照）
    #   ・メタデータ確認 -> OData詳細URL（$expand=Attributes,Assets で返る内容をJSONで確認できる）
